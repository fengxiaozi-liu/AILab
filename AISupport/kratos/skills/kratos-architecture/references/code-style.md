...
- `internal/data/ent/**` 生成产物默认只通过生成链路更新，不作为正式修复入口手工维护
- 不允许通过删除、局部修补或临时改坏生成产物来绕过 `schema` 设计问题或生成错误
- 生成目录应始终保持“可整体再生”的一致状态，不应长期处于部分文件已更新、部分文件缺失或关系失配的中间态
- 如果 `ent` 相关错误来自 `schema` 变更，应优先回到 `schema`、生成顺序与引用切换顺序上修正，而不是在生成代码层打补丁
- 任何需要清理旧字段、旧关系或旧生成引用的动作，都应放在成功生成并完成引用切换之后
