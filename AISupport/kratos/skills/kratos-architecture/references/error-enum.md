...
枚举与稳定值域用于承载项目内可复用、可判断、可展示的共享语义，覆盖状态、原因、结果、类型、稳定协议常量，以及查询装配、关联加载、聚合展开、共享过滤、统一状态映射等稳定语义常量。
...
- 如果一个常量会在多个 `repo / usecase / service / assembler / proto convert` 中重复使用，或表达的是稳定业务语义而不是单次实现细节，也应按业务域级共享常量处理
