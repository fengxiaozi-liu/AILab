...
- 稳定错误语义必须集中定义，`not found` 必须表达真实未找到，不得伪装成功
- 稳定值域、协议字面量与第三方固定状态值进入业务消费链前，必须收敛为 enum、typed const 或共享定义
- 不要让裸字符串、裸数字长期停留在 `biz` 中参与状态机、分支或错误判断
- 第三方 DTO 可以保留原始字段形态，但项目内部消费对象应尽快转为项目语义
- error与enum必选按照示例模板形式来完成
