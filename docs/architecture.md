...
本仓库是 AI 支持资产仓库，核心目标是维护 `AISupport/` 下的 Agent 资产，并通过 `utils/ferryPilot/` 提供跨平台安装工具。
...
| `utils/ferryPilot/` | 安装 `AISupport` 资产的 Go CLI 工具 |
| `docs/` | 项目架构和发布产物说明 |
...
`ferryPilot` 位于 `utils/ferryPilot/`，用于把 `AISupport/<package>` 安装到目标 Agent 的全局目录或当前项目目录。

主要模块：

| 模块 | 职责 |
| --- | --- |
| `cmd/ferryPilot` | CLI 入口 |
...
1. 用户运行 `ferryPilot -p speckit` 或 `ferryPilot -g speckit`。
...
推送匹配 `v*` 的 tag 后，GitHub Actions 会为 Windows、Linux、macOS 交叉编译 `ferryPilot`，并把单文件可执行产物上传到 GitHub Release。
